...
import Test.Hspec (shouldBe, describe, it, Spec, hspec)
import Text.Parsec (parse, ParseError)
import Lib (doParse, LispVal(..))
  ...
  describe "Parser" $ do
    ...
    describe "parseList" $ do
      ...
      it "parses a list with multiple elements" $
        doParse "(a b c)" `shouldBe` 
          Right (List [Atom "a", Atom "b", Atom "c"])
    ...
    describe "parseDottedList" $ do
      it "parses a simple dotted list" $
        doParse "(a . b)" `shouldBe` 
          Right (DottedList [Atom "a"] (Atom "b"))
      
      it "parses a dotted list with multiple head elements" $
        doParse "(a b c . d)" `shouldBe` 
          Right (DottedList [Atom "a", Atom "b", Atom "c"] (Atom "d"))
    ...
    describe "complex expressions" $ do
      it "parses a complex expression" $
        doParse "(define (factorial n) (if (= n 0) 1 (* n (factorial (- n 1)))))" `shouldBe`
          Right (List [Atom "define", 
